...
Learn how Kynno handles pending state queries. {synopsis}
...
Kynno is designed quite differently on this front as there is no concept of a "pending state".
Kynno uses [Tendermint Core](https://docs.tendermint.com/) BFT consensus which provides instant
...
first-in-first-out (FIFO) fashion. Transactions on Kynno cannot be ordered or cherry picked out from the Tendermint node [mempool](https://docs.tendermint.com/master/tendermint-core/mempool/).

## Pending State Queries

Kynno will make queries which will account for any unconfirmed transactions present in a node's
