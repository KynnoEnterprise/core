...
MY_VALIDATOR_ADDRESS=$(kynnod keys show mykey -a)
```
...
::: warning
**NOTE**: Cosmos `secp256k1` keys are not supported on Kynno due to compatibility issues with Ethereum transactions.
:::
...
yes $KEYPASSWD | kynnod keys add me
yes $KEYPASSWD | kynnod keys show me
# start kynnod with keyring-backend flag
kynnod --keyring-backend=file start
```
