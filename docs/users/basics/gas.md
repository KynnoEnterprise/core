...
loops by allocating a small amount of monetary value into the system. A unit of gas, usually in the form of a fraction of the native coin, is consumed for every operation on the EVM and requires a user to pay for these operations. These operations consist in state transitions such as sending a transaction or calling a contract.
...
Kynno is an EVM-compatible chain that supports Ethereum Web3 tooling. For this reason, gas
...
+++ https://github.com/ahmedoubadi/ethermint/tree/main/x/evm/keeper/state_transition.go#L188
...
For this same reason, in Kynno it is possible to send transactions with `0` fees for transaction
...
(i.e `ValidateBasic`) function as well as on the custom `AnteHandler` defined by Kynno.
...
+++ https://github.com/ahmedoubadi/ethermint/tree/main/x/evm/keeper/grpc_query.go#L100
