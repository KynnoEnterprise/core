...
This command creates a 4-node network using the `kynnodnode` Docker image.
The ports for each node are found in this table:

| Node ID          | P2P Port | Tendermint RPC Port | REST/ Ethereum JSON-RPC Port | WebSocket Port |
|------------------|----------|---------------------|------------------------------|----------------|
| `kynnonode0` | `26656`  | `26657`             | `8545`                       | `8546`         |
| `kynnonode1` | `26659`  | `26660`             | `8547`                       | `8548`         |
| `kynnonode2` | `26661`  | `26662`             | `8549`                       | `8550`         |
| `kynnonode3` | `26663`  | `26664`             | `8551`                       | `8552`         |
...
```bash
...
Creating network "kynno_localnet" with driver "bridge"
Creating kynnodnode0 ... done
Creating kynnodnode2 ... done
Creating kynnodnode1 ... done
Creating kynnodnode3 ... done
```
...
calling the `kynnod testnet` command. This outputs a handful of files in the
`./build` directory:
...
Each `./build/nodeN` directory is mounted to the `/kynnod` directory in each container.
...
```bash
# node 0: daemon logs
docker exec kynnodnode0 tail kynnod.log
...
To interact with `kynnod` and start querying state or creating txs, you use the
`kynnod` directory of any given node as your `home`, for example:

```bash
kynnod keys list --home ./build/node0/kynnod
```
...
::: tip
**Note**: Each node's seed is located at `./build/nodeN/kynnod/key_seed.json` and can be restored to the CLI using the `kynnod keys add --restore` command
:::
...
BINARY=kynno make localnet-start
```
