...
Learn how to run and setup the JSON-RPC server on Kynno. {synopsis}
...
```bash
kynnod start --json-rpc.api eth,txpool,personal,net,debug,web3,miner
```
...
```bash
# set gas cap to 85M
kynnod start --json-rpc.gas-cap 85000000000

# set gas cap to infinite (=0)
kynnod start --json-rpc.gas-cap 0
```
