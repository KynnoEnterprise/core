...
The JSON-PRC Server provides an API that allows you to connect to the Kynno blockchain and interact with the EVM. This gives you direct access to reading Ethereum-formatted transactions or sending them to the network which otherwise wouldn't be possible on a Cosmos chain, such as Kynno.
...
JSON-RPC is provided on multiple transports. Kynno supports JSON-RPC over HTTP and WebSocket. Transports must be enabled through command-line flags or through the `app.toml` configuration file.

## Web3 Support

Kynno supports all standard web3 JSON-RPC APIs. You can find documentation for these APIs on the [`JSON-RPC Methods`](./endpoints.md) page.
...
When requests are made that act on the state of Kynno, the last default block parameter determines the height of the block.
