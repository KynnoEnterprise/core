...
The Kynno supports different clients in order to support Cosmos and Ethereum transactions
and queries:

|                                                        | Description                                                                          | Default Port |
| ------------------------------------------------------ | ------------------------------------------------------------------------------------ | ------------ |
| **Cosmos [gRPC](#cosmos-grpc)**                        | Query or send Kynno transactions using gRPC                                          | `9090`       |
| **Cosmos REST ([gRPC-Gateway](#cosmos-grpc-gateway))** | Query or send Kynno transactions using an HTTP RESTful API                           | `9091`       |
...
| **Command Line Interface ([CLI](#cli))**               | Query or send Kynno transactions using your Terminal or Console.                     | N/A          |

## Cosmos gRPC

Kynno exposes gRPC endpoints for all the integrated Cosmos SDK modules. This makes it easier for
...
See the list of supported gRPC-Gateway API endpoints for the Kynno testnet [here](https://api.kynno.dev/swagger/).

## Ethereum JSON-RPC

<!-- TODO: Link JSON-RPC docs -->

Kynno supports most of the standard [JSON-RPC APIs](./json-rpc/server.md) to connect with existing Ethereum-compatible web3 tooling.
...
Check the `Events` page to obtain the event list of each supported module on Kynno.
:::
...
Users can use the `{{ $themeConfig.project.binary }}` binary to interact directly with an Kynno node
though the CLI.
...
- **Transactions**: `{{ $themeConfig.project.binary }} tx`
    ...
    The list of available commands, as of `v1.0.0`, are:
