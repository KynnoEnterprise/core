...
func (k Keeper) OnRecvPacket(
	...
	// readable prefix (HRP) of the sender to `kynno1`
	...
	// supported on kynno. To prevent funds getting stuck, return an error, unless
	...
	if sameAddress && !fromEVMChain {
		...
		case senderRecordFound && !senderClaimsRecord.HasClaimedAny():
			...
			return channeltypes.NewErrorAcknowledgement(
				sdkerrors.Wrapf(
					kynno.ErrKeyTypeNotSupported, "receiver address %s is not a valid ethereum address", recipientBech32,
				),
			)
		default:
