...
var (
	now  = time.Now().UTC()
	acc1 = sdk.MustAccAddressFromBech32("kynno1hxey68h286z7504psv9nh9x4fjn3ntsfd373zj")
	acc2 = sdk.MustAccAddressFromBech32("kynno1glxa8cl3shzk33586h0ydthfen59m4zky6r6rg")
)
...
func (suite *GenesisTestSuite) TestClaimInitGenesis() {
	testCases := []struct {
		...
		},
		{
			...
			func() {
				coins := sdk.NewCoins(sdk.NewCoin("akynno", sdk.NewInt(2_800)))
		...
		},
		{
			...
			func() {
				coins := sdk.NewCoins(sdk.NewCoin("akynno", sdk.NewInt(400)))
				err := testutil.FundModuleAccount(suite.app.BankKeeper, suite.ctx, types.ModuleName, coins)
				suite.Require().NoError(err)
			},
			false,
