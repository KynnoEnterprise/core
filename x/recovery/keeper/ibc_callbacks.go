...
import (
	...
	"github.com/kynnoenterprise/core/ibc"
	kynno "github.com/kynnoenterprise/core/types"
...
//   - sends over all Kynno native tokens
//
...
func (k Keeper) OnRecvPacket(
	...
	// Get addresses in `kynno1` and the original bech32 format
	...
	// a sender's own account on Kynno (sender == recipient)
	...
	if account != nil && kynno.IsSupportedKey(account.GetPubKey()) {
		return ack
	}
	...
	k.bankKeeper.IterateAccountBalances(ctx, recipient, func(coin sdk.Coin) (stop bool) {
		...
		err = k.transferKeeper.SendTransfer(
			...
			recipient,                 // recipient is the address in the Kynno chain
...
// the IBC denomination, i.e port and channel on Kynno for the voucher. It
// returns an error if:
