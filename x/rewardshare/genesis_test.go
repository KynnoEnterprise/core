...
func (suite *GenesisTestSuite) TestRewardshareInitGenesis() {
	testCases := []struct {
		name     string
		genesis  types.GenesisState
		expPanic bool
		...
		{
			"custom genesis - rewardshare disabled",
			types.GenesisState{
				Params: types.Params{
					EnableRewardshare:            false,
		...
		suite.Run(fmt.Sprintf("Case %s", tc.name), func() {
			...
			if tc.expPanic {
				suite.Require().Panics(func() {
					rewardshare.InitGenesis(suite.ctx, suite.app.RewardshareKeeper, tc.genesis)
				})
			} else {
...
func (suite *GenesisTestSuite) TestRewardshareExportGenesis() {
	rewardshare.InitGenesis(suite.ctx, suite.app.RewardshareKeeper, suite.genesis)

	genesisExported := rewardshare.ExportGenesis(suite.ctx, suite.app.RewardshareKeeper)
