...
var _ = Describe("Fee distribution:", Ordered, func() {
	...
	BeforeAll(func() {
		...
		params = s.app.RewardshareKeeper.GetParams(s.ctx)
		params.EnableRewardshare = true
		s.app.RewardshareKeeper.SetParams(s.ctx, params)
	...
	Context("with rewardshare param disabled", func() {
		var registeredContract common.Address

		BeforeAll(func() {
			// rewardshare registered before disabling params
			...
			fee, isRegistered := s.app.RewardshareKeeper.GetRewardshare(s.ctx, registeredContract)
		...
		It("should not allow new contract registrations", func() {
			...
			Expect(
				strings.Contains(res.GetLog(),
					"rewardshare module is disabled by governance"),
			).To(BeTrue())
			s.Commit()

			_, isRegistered := s.app.RewardshareKeeper.GetRewardshare(s.ctx, contractAddress)
		...
		It("should not allow rewardshare updates for previously registered contracts", func() {
			withdrawerAddress := sdk.AccAddress(tests.GenerateAddress().Bytes())
			msg := types.NewMsgUpdateRewardshare(
				registeredContract,
				deployerAddress,
				withdrawerAddress,
			)
		...
		It("should not allow cancellations of previously registered contracts", func() {
			msg := types.NewMsgCancelRewardshare(registeredContract, deployerAddress)
			res := deliverTx(deployerKey, nil, msg)
	...
	Context("with rewardshare param enabled", func() {
		BeforeEach(func() {
		...
		Describe("Registering a contract for receiving tx fees", func() {
			Context("with an empty withdrawer address", Ordered, func() {
				...
				It("should be possible", func() {
					...
					fee, isRegistered := s.app.RewardshareKeeper.GetRewardshare(s.ctx, contractAddress)
					Expect(isRegistered).To(Equal(true))
		...
		Describe("Interacting with a registered rewardshare contract", func() {
			var contractAddress common.Address
			var nonce uint64
			...
			Context("with a 50/50 validators-developers rewardshare", func() {
				BeforeEach(func() {
					params = s.app.RewardshareKeeper.GetParams(s.ctx)
			...
			Context("with a 100/0 validators-developers rewardshare", func() {
				BeforeEach(func() {
					params = s.app.RewardshareKeeper.GetParams(s.ctx)
			...
			Context("with a 0/100 validators-developers rewardshare", func() {
				BeforeEach(func() {
					params = s.app.RewardshareKeeper.GetParams(s.ctx)
		...
		Describe("Updating registered rewardshare", func() {
			Context("with a withdraw address that is different from the deployer address", Ordered, func() {
				...
				It("should update rewardshare successfully", func() {
					msg := types.NewMsgUpdateRewardshare(
						contractAddress,
						deployerAddress,
						withdrawerAddress,
			...
			Context("with a withdraw address equal to the deployer's address", func() {
				...
				It("should not update rewardshare", func() {
					msg := types.NewMsgUpdateRewardshare(
						contractAddress,
						deployerAddress,
						deployerAddress,
					...
					Expect(
						strings.Contains(res.GetLog(),
							"rewardshare already exists for given contract"),
		...
		Describe("Canceling a rewardshare registration", func() {
			When("the registered rewardshare exists", Ordered, func() {
				var contractAddress common.Address
				var nonce uint64
				...
				It("should be possible", func() {
					msg := types.NewMsgCancelRewardshare(contractAddress, deployerAddress)
					res := deliverTx(deployerKey, nil, msg)
			...
			When("the rewardshare does not exist", func() {
...
func registerFee(
	...
	msg := types.NewMsgRegisterRewardshare(*contractAddress, deployerAddress, withdrawerAddress, nonces)
	...
	if res.IsOK() {
		registerEvent := res.GetEvents()[8]
		Expect(registerEvent.Type).To(Equal(types.EventTypeRegisterRewardshare))
