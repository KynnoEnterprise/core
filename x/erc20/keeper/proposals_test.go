...
import (
	...
	"github.com/kynnoenterprise/core/x/erc20/keeper"
	"github.com/kynnoenterprise/core/x/erc20/types"
...
func (suite KeeperTestSuite) TestRegisterCoin() {
	...
	testCases := []struct {
		...
		{
			"evm denom registration - akynno",
			func() {
				metadata.Base = "akynno"
				err := suite.app.BankKeeper.MintCoins(suite.ctx, inflationtypes.ModuleName, sdk.Coins{sdk.NewInt64Coin(metadata.Base, 1)})
				suite.Require().NoError(err)
			},
			false,
		...
		{
			"evm denom registration - wkynno",
			func() {
				metadata.Base = "wkynno"
				err := suite.app.BankKeeper.MintCoins(suite.ctx, inflationtypes.ModuleName, sdk.Coins{sdk.NewInt64Coin(metadata.Base, 1)})
				suite.Require().NoError(err)
			},
			false,
