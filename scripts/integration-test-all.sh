...
## default port prefixes for kynnod
NODE_P2P_PORT="2660"
NODE_PORT="2663"
NODE_RPC_PORT="2666"
...
init_func() {
    "$PWD"/build/kynnod keys add $KEY"$i" --keyring-backend test --home "$DATA_DIR$i" --no-backup --algo "eth_secp256k1"
    "$PWD"/build/kynnod init $MONIKER --chain-id $CHAINID --home "$DATA_DIR$i"
    "$PWD"/build/kynnod add-genesis-account \
    "$("$PWD"/build/kynnod keys show "$KEY$i" --keyring-backend test -a --home "$DATA_DIR$i")" 1000000000000000000akynno,1000000000000000000stake \
    --keyring-backend test --home "$DATA_DIR$i"
    "$PWD"/build/kynnod gentx "$KEY$i" 1000000000000000000stake --chain-id $CHAINID --keyring-backend test --home "$DATA_DIR$i"
    "$PWD"/build/kynnod collect-gentxs --home "$DATA_DIR$i"
    "$PWD"/build/kynnod validate-genesis --home "$DATA_DIR$i"
...
start_func() {
    ...
    "$PWD"/build/kynnod start --pruning=nothing --rpc.unsafe \
    ...
    KYNNO_PID=$!
    echo "started kynno node, pid=$KYNNO_PID"
    # add PID to array
    arr+=("$KYNNO_PID")
...
if [[ -z $TEST || $TEST == "rpc" ||  $TEST == "pending" ]]; then
    ...
    for i in $(seq 1 "$TEST_QTD"); do
        ...
        echo "going to test kynno node $HOST_RPC ..."
...
stop_func() {
    KYNNO_PID=$i
    echo "shutting down node, pid=$KYNNO_PID ..."
