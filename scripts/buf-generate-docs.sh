...
# also make sure that you are added to the Kynno organization on buf
...
buf mod update $KYNNOPROTO

# then, push the kynno proto module and documentation 
buf push $KYNNOPROTO

# two commit addresses should be printed to the command line
# the kynno documentation will have links to the third party documentation
