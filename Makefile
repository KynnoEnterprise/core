...
SIMAPP = ./app
HTTPS_GIT := https://github.com/kynnoenterprise/core.git
...
ldflags = -X github.com/cosmos/cosmos-sdk/version.Name=kynno \
          -X github.com/cosmos/cosmos-sdk/version.AppName=$(KYNNO_BINARY) \
...
build-docker:
	...
	$(DOCKER) rm kynno || true
	# create a new container from the latest image
	$(DOCKER) create --name kynno -t -i ${DOCKER_IMAGE}:latest kynno
	# move the binaries to the ./build directory
	mkdir -p ./build/
	$(DOCKER) cp kynno:/usr/bin/kynnod ./build/
...
godocs:
	@echo "--> Wait a few seconds and visit http://localhost:6060/pkg/github.com/kynno/kynno/types"
	godoc -http=:6060
...
test-sim-custom-genesis-fast:
	...
	@echo "By default, ${HOME}/.$(KYNNO_DIR)/config/genesis.json will be used."
	@go test -mod=readonly $(SIMAPP) -run TestFullAppSimulation -Genesis=${HOME}/.$(KYNNO_DIR)/config/genesis.json \
...
test-sim-custom-genesis-multi-seed: runsim
	...
	@echo "By default, ${HOME}/.$(KYNNO_DIR)/config/genesis.json will be used."
	@$(BINDIR)/runsim -Genesis=${HOME}/.$(KYNNO_DIR)/config/genesis.json -SimAppPkg=$(SIMAPP) -ExitOnFail 400 5 TestFullAppSimulation
...
localnet-start: localnet-stop localnet-build
	@if ! [ -f build/node0/$(KYNNO_BINARY)/config/genesis.json ]; then docker run --rm -v $(CURDIR)/build:/kynno:Z kynno/node "./kynnod testnet init-files --v 4 -o /kynno --keyring-backend=test --starting-ip-address 192.167.10.2"; fi
	docker-compose up -d
...
localnet-unsafe-reset:
	docker-compose down
ifeq ($(OS),Windows_NT)
	@docker run --rm -v $(CURDIR)\build\node0\kynnod:/kynno\Z kynno/node "./kynnod tendermint unsafe-reset-all --home=/kynno"
	@docker run --rm -v $(CURDIR)\build\node1\kynnod:/kynno\Z kynno/node "./kynnod tendermint unsafe-reset-all --home=/kynno"
	@docker run --rm -v $(CURDIR)\build\node2\kynnod:/kynno\Z kynno/node "./kynnod tendermint unsafe-reset-all --home=/kynno"
	@docker run --rm -v $(CURDIR)\build\node3\kynnod:/kynno\Z kynno/node "./kynnod tendermint unsafe-reset-all --home=/kynno"
else
	@docker run --rm -v $(CURDIR)/build/node0/kynnod:/kynno:Z kynno/node "./kynnod tendermint unsafe-reset-all --home=/kynno"
	@docker run --rm -v $(CURDIR)/build/node1/kynnod:/kynno:Z kynno/node "./kynnod tendermint unsafe-reset-all --home=/kynno"
	@docker run --rm -v $(CURDIR)/build/node2/kynnod:/kynno:Z kynno/node "./kynnod tendermint unsafe-reset-all --home=/kynno"
	@docker run --rm -v $(CURDIR)/build/node3/kynnod:/kynno:Z kynno/node "./kynnod tendermint unsafe-reset-all --home=/kynno"
endif
...
PACKAGE_NAME:=github.com/kynno/kynno
